...
def scenario_broken_staging():
    print("\n====================")
    print("SCENARIO: broken_staging")
    print("====================")
    world = make_world()
    preload_u1r(world)
    ...
    print_state("after tick 6 (LL at d1R)", world)

    step(world, {"L1": "mR", "L2": "mR", "R1": "u1R"})
    print_state("after tick 7 (dyad bundle at mR)", world)

    step(world, {"L1": "u1R", "L2": "mL", "R1": "u1R"})
    print_state("after tick 8 (dyad meets resident at u1R)", world)

    step(world, {"L1": "mR", "L2": "u1L", "R1": "mR"})
    print_state("after tick 9 (staging broken; pair leaves u1R)", world)

    step(world, {"L1": "u1R", "L2": "u2L", "R1": "u1R"})
    print_state("after tick 10 (pair re-forms at u1R without third)", world)

    step(world, {"L1": "u1R", "L2": "u1L", "R1": "u1R"})
    print_state("after tick 11 (third advances to u1L)", world)

    step(world, {"L1": "u1R", "L2": "u1R", "R1": "u1R"})
    print_state("after tick 12 (third arrives after broken staging)", world)

    step(world, {"L1": "mR", "L2": "mR", "R1": "mR"})
    print_state("after tick 13 (inspect post-event export)", world)


def main():
    scenario_broken_staging()
