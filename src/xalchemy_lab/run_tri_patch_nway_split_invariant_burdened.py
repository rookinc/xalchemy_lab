...
def run_event(
    world: World,
    node: str,
) -> tuple[tuple[int, int, int], tuple[int, int, int], tuple[int, int, int, int, int], str]:
    world.tick += 1
...
def main() -> None:
    print("\n====================")
    print("N-WAY SPLIT INVARIANT SWEEP (BURDENED)")
    print("====================")
    print("Probe whether the clean-manifold split law survives once burden > 0.\n")

    cases = [
        {
            "name": "coherent_positive_mismatch",
            "node": "u1R",
            "signs": ("+", "+", "+"),
            "mismatch": (1, 0, 0),
            "totals": [(1, 1, 1), (2, 2, 2)],
    ...
    for case in cases:
        ...
        for total in case["totals"]:
            ...
            for n in (2, 3):
                ...
                expected_clean_delta = (-(n - 1), 0, 0, +(n - 1), 0)
                print(f"    {n}-way nonzero splits checked = {len(seqs)}")
                print(f"    clean-sector target delta      = {expected_clean_delta}")

                all_hit_clean_target = True
                ...
                for seq in seqs:
                    ...
                    if delta != expected_clean_delta:
                        all_hit_clean_target = False
                    if ledger_match:
                ...
                print(f"      all hit clean target? {all_hit_clean_target}")
