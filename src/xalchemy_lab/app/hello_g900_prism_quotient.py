...
def main() -> None:
    ...
    print("G900 PRISM QUOTIENT PROBE")
    print("========================")
    ...
    for i in range(len(class_nodes)):
        for j in range(i + 1, len(class_nodes)):
            if support[i][j]:
                print(f"  {class_nodes[i]} -- {class_nodes[j]}  weight={weights[i][j]}")

    expected_prism_edges = {
        frozenset(((0, 0), (1, 0))),
        frozenset(((1, 0), (2, 0))),
        frozenset(((2, 0), (0, 0))),
        frozenset(((0, 1), (1, 1))),
    ...
    print(f"  expected edge count = {len(expected_prism_edges)}")
    print(f"  exact match         = {observed_edges == expected_prism_edges}")
