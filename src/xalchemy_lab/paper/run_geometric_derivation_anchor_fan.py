        ...
        for q1, e01 in nbrs[q0]:
            ...
            for q2, e12 in nbrs[q1]:
                ...
                for q3, e23 in nbrs[q2]:
                    ...
                    # normalize so anchor appears first as q0->q1
                    if cycle_edges[0] != ANCHOR:
                        for i in range(4):
                            rot_edges = cycle_edges[i:] + cycle_edges[:i]
                            rot_verts = verts[i:4] + verts[0:i+1]
                            if rot_edges[0] == ANCHOR:
                                cycle_edges = rot_edges
                                verts = rot_verts
                                break

                    fan.append((verts, cycle_edges, parity))

    fan.sort(key=lambda x: (x[2], x[1]))

    print("\n====================")
    print("GEOMETRIC DERIVATION: ANCHOR FAN")
    print("====================\n")

    for verts, edges, parity in fan:
        tag = "EVEN" if parity == 0 else "ODD"
        print(f"{tag:4s}  anchor_path={verts[0]}->{verts[1]}  cycle={verts}  edges={edges}")

    print()
    print("sector packets beyond the anchor")
    for verts, edges, parity in fan:
        packet = edges[1:]
        tag = "EVEN" if parity == 0 else "ODD"
        print(f"  {tag:4s}  {packet}")
