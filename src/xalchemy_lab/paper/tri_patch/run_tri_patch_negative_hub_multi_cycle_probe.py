...
def make_world(stress_vec: tuple[int, int, int]) -> World:
    return World(
        turtles={
            "L1": Turtle(name="L1", chirality="L", node="d1R", carry_sign="-", carried_stress=stress_vec[0]),
            "L2": Turtle(name="L2", chirality="L", node="d1R", carry_sign="-", carried_stress=stress_vec[1]),
            "R1": Turtle(name="R1", chirality="R", node="d1R", carry_sign="-", carried_stress=stress_vec[2]),
        }
    )
...
def full_cycle(world: World) -> dict[str, object]:
    ...
    d1r_first = world.collisions[-1].face_event
    ...
    u1r = world.collisions[-1].face_event

    # 4. export back through mR
    step(world, {"L1": "mR", "L2": "mR", "R1": "mR"})
    export_2 = world.collisions[-1].face_event

    # 5. return closure at d1R
    step(world, {"L1": "d1R", "L2": "d1R", "R1": "d1R"})
    d1r_return = world.collisions[-1].face_event
...
def run_seed(label: str, seed: tuple[int, int, int]) -> None:
    ...
    for cycle in range(1, N_CYCLES + 1):
        ...
        print(f"\ncycle {cycle}")
        print(f"  d1R first       = {result['d1r_first']}")
        print(f"  export 1        = {result['export_1']}")
        print(f"  u1R             = {result['u1r']}")
        print(f"  export 2        = {result['export_2']}")
        print(f"  d1R return      = {result['d1r_return']}")
...
def main() -> None:
    print("\n====================")
    print("NEGATIVE-HUB MULTI-CYCLE PROBE")
    print("====================")
    print("Repeated full loops:")
    print("d1R closure -> mR export -> u1R closure -> mR export -> d1R return")
