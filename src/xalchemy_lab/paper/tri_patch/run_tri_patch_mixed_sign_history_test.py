...
def scenario_mixed_sign_history():
    print("\n====================")
    print("SCENARIO: mixed_sign_history")
    print("====================")
    world = make_world()
    preload_u1r(world)
    ...
    # Send only L1 through d1R to pick up negative sign history
    step(world, {"L1": "mL", "L2": "mL", "R1": "u1R"})
    print_state("after tick 3 (into mL)", world)

    step(world, {"L1": "mR", "L2": "u1L", "R1": "u1R"})
    print_state("after tick 4 (L1 crosses, L2 holds +)", world)

    step(world, {"L1": "d1R", "L2": "u1L", "R1": "u1R"})
    print_state("after tick 5 (L1 reaches d1R lane)", world)

    step(world, {"L1": "d1R", "L2": "u2L", "R1": "u1R"})
    print_state("after tick 6 (L1 holds alone at d1R)", world)

    # Return L1 to u1R with negative sign history while L2 still has positive history
    step(world, {"L1": "mR", "L2": "u1L", "R1": "u1R"})
    print_state("after tick 7 (L1 returns via mR, L2 re-approaches u1L)", world)

    step(world, {"L1": "u1R", "L2": "u1L", "R1": "u1R"})
    print_state("after tick 8 (L1 meets resident at u1R)", world)

    # Advance L2 as delayed third with different sign history
    step(world, {"L1": "u1R", "L2": "mL", "R1": "u1R"})
    print_state("after tick 9 (L2 into mL)", world)

    step(world, {"L1": "u1R", "L2": "mR", "R1": "u1R"})
    print_state("after tick 10 (L2 into mR)", world)

    step(world, {"L1": "u1R", "L2": "u1R", "R1": "u1R"})
    print_state("after tick 11 (mixed-history triad assembled at u1R)", world)

    step(world, {"L1": "mR", "L2": "mR", "R1": "mR"})
    print_state("after tick 12 (inspect post-event export)", world)


def main():
    scenario_mixed_sign_history()
