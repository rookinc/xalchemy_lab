...
def scenario_coherent_sign_low_mismatch():
    print("\n====================")
    print("SCENARIO: coherent_sign_low_mismatch")
    print("====================")
    world = make_world()
    preload_u1r(world)
    ...
    # Minimal burden: only one turtle has mismatch 1
    world.turtles["L1"].mismatch_count = 1
    print_state("after manual seed (only L1 has mismatch=1)", world)
    ...
    print_state("after tick 5 (coherent pair at u1R)", world)

    step(world, {"L1": "u1R", "L2": "mL", "R1": "u1R"})
    print_state("after tick 6", world)

    step(world, {"L1": "u1R", "L2": "mR", "R1": "u1R"})
    print_state("after tick 7", world)

    step(world, {"L1": "u1R", "L2": "u1R", "R1": "u1R"})
    print_state("after tick 8 (coherent low-mismatch triad at u1R)", world)

    step(world, {"L1": "mR", "L2": "mR", "R1": "mR"})
    print_state("after tick 9", world)


def main():
    scenario_coherent_sign_low_mismatch()
