...
def make_world(node: str, hub_sign: str) -> World:
    return World(
        turtles={
            "L1": Turtle(name="L1", chirality="L", node=node, carry_sign=hub_sign),
            "L2": Turtle(name="L2", chirality="L", node=node, carry_sign=hub_sign),
            "R1": Turtle(name="R1", chirality="R", node=node, carry_sign=hub_sign),
        }
    )
...
def configure_case(
    ...
    for name, stress, mismatch in zip(
        ["L1", "L2", "R1"], stress_triplet, mismatch_triplet
    ):
        t = world.turtles[name]
        t.carried_stress = stress
        t.mismatch_count = mismatch
        t.shared_tokens = []
        t.face_tokens = []
        t.seen_nodes = [t.node]


def expected_partial_injection(stress_triplet: tuple[int, int, int]) -> tuple[int, int, int]:
    return tuple(s + 1 for s in stress_triplet)
...
def print_case_result(
    ...
    expected = expected_partial_injection(stress_triplet)
    observed = (
    ...
    print(f"  injection_matches = {observed == expected}")
    print(
        "  ledger_delta      = "
...
def run_suite(hub: str, sign: str) -> None:
    ...
    print("")
    print("====================")
    print(f"PARTIAL TENSION INJECTION SUITE @ {hub}")
    print("====================")
    print(f"All cases are coherent {sign}{sign}{sign} with positive burden.")
    print("Probe: outgoing_stress_i == incoming_stress_i + 1")

    for label, stress_triplet, mismatch_triplet in cases:
        world = make_world(hub, sign)
        configure_case(world, stress_triplet, mismatch_triplet)
        before = deepcopy(world)
        apply_collision(world, hub, ["L1", "L2", "R1"])
        print_case_result(
            label, hub, sign, stress_triplet, mismatch_triplet, before, world
        )


def main() -> None:
    print("\n====================")
    print("SCENARIO: partial_tension_injection_probe")
    print("====================")
    run_suite("u1R", "+")
    run_suite("d1R", "-")
