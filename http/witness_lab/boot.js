import { renderWitnessGraph } from "./graph_view.js";
...
function renderGraph() {
  renderWitnessGraph(els.graphCanvas, app.structure);
}

function syncViewMode() {
  const mode = els.viewMode.value;
  els.consolePanel.classList.toggle("is-hidden", mode !== "console");
  els.graphPanel.classList.toggle("is-hidden", mode !== "graph");
  if (mode === "graph") {
    renderGraph();
  }
}
...
async function loadStructure(frame, phase) {
  const data = await fetchJson(`/api/witness/structure?frame=${frame}&phase=${phase}`);
  app.structure = data.payload;
}
...
async function loadState(frame = 0, phase = 0) {
  ...
  await loadStructure(frame, phase);
  syncViewMode();
}
...
async function applyOp(op) {
  ...
  pushHistory(app.current);
  renderConsole(app.current);
  await loadStructure(app.current.frame, app.current.phase);
  if (els.viewMode.value === "graph") {
    renderGraph();
  }
}
