...
def sample_depth3(seed: list[str], vocab: list[str], sample_size: int, random_seed: int) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    sample_depth = payload.get("sample_depth", 3)
    sample_size = payload.get("sample_size", 400)
    random_seed = payload.get("random_seed", 17)
    max_steps = payload.get("max_steps", 8)

    if sample_depth != 3:
        raise SystemExit("this script currently supports sample_depth = 3 only")

    samples = sample_depth3(seed, vocab, sample_size, random_seed)
