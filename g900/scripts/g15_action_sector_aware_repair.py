...
def edited_positions_from_label(label: str) -> set[int]:
    out = set()
        ...
        if part.startswith("edit_pos"):
            ...
            for ch in rest:
                if ch.isdigit():
                    ...
                    break
            if pos:
                out.add(int("".join(pos)))
    return out


def weak_sector_penalty(label: str, weak_positions: set[int]) -> int:
    return len(edited_positions_from_label(label) & weak_positions)
...
def choose_best_child(current: dict[str, Any], vocab: list[str], r: int, weak_positions: set[int], alpha: float) -> dict[str, Any] | None:
    ...
    for item in one_edit_variants(current["cycle"], vocab):
        ...
        p = weak_sector_penalty(row["label"], weak_positions)
        score = d + alpha * p
        candidates.append((score, d, p, row["label"], row))
...
def walk(start: dict[str, Any], vocab: list[str], r: int, weak_positions: set[int], alpha: float, max_steps: int) -> dict[str, Any]:
    path = [start]
    seen = {tuple(start["cycle"])}
    current = start

    for _ in range(max_steps):
        nxt = choose_best_child(current, vocab, r, weak_positions, alpha)
        if nxt is None:
            break
        key = tuple(nxt["cycle"])
        if key in seen:
...
def main() -> int:
    ...
    ap.add_argument("--weak-positions", default="0,1,4,5")
    args = ap.parse_args()
    ...
    weak_positions = {int(x) for x in args.weak_positions.split(",") if x.strip()}
    ...
    out = {
        ...
        "alpha": args.alpha,
        "weak_positions": sorted(weak_positions),
